...
class DQNAgent:
    def __init__(self, environment, trained_model=None):
        ...
        # Initialize neural network model and parameter
        ...
        self.episode_pure_exploration = 25
    ...
    def store(self, state, action, reward, next_state, done):
        ...
        new_state = np.reshape(next_state, [1, self.state_size])
    ...
    def action(self, state, reward, done, episode, training=True):
        ...
        # Return next action
            ...
            # Train agent
            if training:
                if episode >= self.episode_pure_exploration:
                    self.replay()
